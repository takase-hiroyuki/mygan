...
export async function executeGenericPayment(supabase, currentUserId, amountStr) {
    ...
    if (isNaN(inputAmount) || inputAmount < 0) {
        return await insertSystemMessage(supabase, playerName, "金額が一致しません。入力しなおしてください。"); 
    }
    ...
    if (userError || roomError || !userData || !roomData) {
        return await insertSystemMessage(supabase, playerName, "データの取得に失敗しました。"); 
    }
    ...
    if (flags.is_negative_cash_flow && (flags.pending_paydays || 0) > 0) {
        ...
        try {
            ...
            if (result && result.status === 'error') {
                await insertSystemMessage(supabase, playerName, `支払いエラー: ${result.message}`); 
            ...
            await insertSystemMessage(supabase, playerName, `マイナスキャッシュフローの支払いに失敗しました: ${error.message}`); 
            if (btn) btn.disabled = false;
        }
        return;
    }
    ...
    if (CELLS_DOODAD.includes(position) && flags.is_card_drawn && !flags.is_action_completed) {
        if (!currentCard || currentCard.deck_type !== 'doodad') return await insertSystemMessage(supabase, playerName, "Doodadカード情報が見つかりません。"); 
        ...
        try {
            ...
            await insertSystemMessage(supabase, playerName, `支払いに失敗しました: ${error.message}`); 
            if (btn) btn.disabled = false;
        }
        return;
    }
    ...
    if (CELLS_DOWNSIZED.includes(position) && !flags.is_action_completed) {
        ...
        try {
            ...
            if (result && result.status === 'error') {
                await insertSystemMessage(supabase, playerName, `解雇処理エラー: ${result.message}`); 
                if (btn) btn.disabled = false;
            } else {
                const inputEl = document.getElementById(SEL_G.CARD.INPUT_PAYMENT_AMOUNT);
                if (inputEl) inputEl.value = '';
            ...
            await insertSystemMessage(supabase, playerName, `解雇の支払いに失敗しました: ${error.message}`); 
            if (btn) btn.disabled = false;
        }
        return;
    }
    ...
    if (CELLS_CHARITY.includes(position) && !flags.is_action_completed) {
        ...
        try {
            ...
            if (result && result.status === 'error') {
                await insertSystemMessage(supabase, playerName, `寄付処理エラー: ${result.message}`); 
                if (btn) btn.disabled = false;
            } else {
                const inputEl = document.getElementById(SEL_G.CARD.INPUT_PAYMENT_AMOUNT);
                if (inputEl) inputEl.value = '';
            ...
            await insertSystemMessage(supabase, playerName, `寄付の支払いに失敗しました: ${error.message}`); 
            if (btn) btn.disabled = false;
        }
        return;
    }

    await insertSystemMessage(supabase, playerName, "現在、実行可能な支払いアクションはありません。"); 
}
